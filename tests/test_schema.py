...
def _remove_number_defaults(obj):
    """Remove default values from number fields - same as generate_schema.py does."""
    if isinstance(obj, dict):
        is_number = obj.get("type") == "number"
        if not is_number and "anyOf" in obj:
            is_number = any(
        ...
        if is_number and "default" in obj:
            del obj["default"]

        for value in obj.values():
            _remove_number_defaults(value)
    elif isinstance(obj, list):
        for item in obj:
            _remove_number_defaults(item)
...
def test_schema_file_is_up_to_date(tmp_path):
    ...
    if not schema_path.exists():
        pytest.fail(
            f"Schema file {schema_path} does not exist. Run 'python scripts/generate_schema.py' to create it."
        )

    current_schema = BabelfishConfig.model_json_schema()
    _remove_number_defaults(current_schema)
