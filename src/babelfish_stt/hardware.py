...
class HardwareManager:
    ...
    def probe(self):
        ...
        if self.gpu_info['cuda_available']:
            logging.info(f"GPU Detected: {self.gpu_info['name']} ({self.gpu_info['vram_gb']:.2f} GB VRAM)")
        else:
        ...
        if self.best_mic_index is not None:
            ...
            logging.info(f"Selected best microphone: {mic_name} (index {self.best_mic_index})")
        else:
